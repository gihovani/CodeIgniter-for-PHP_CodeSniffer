...
class CodeIgniter_Sniffs_NamingConventions_ValidVariableNameSniff extends PHP_CodeSniffer_Standards_AbstractVariableSniff
{
    ...
    {
        $isInLowerCase = true;
        $allowedUpperName = array('_SERVER', '_POST', '_GET', '_FILES', '_SESSION');
        if (0 !== strcmp($varName, strtolower($varName)) && !in_array($varName, $allowedUpperName)) {
    ...
    {
        $minLength = 3;
        $allowedShortName = array('str', 'ci', 'tmp', 'ret', 'db', 'id', 'url', 'sql');
